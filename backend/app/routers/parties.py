...
def list_parties(store: InMemoryStore = Depends(get_store)) -> list[Party]:
    return store.list_parties()


@router.post("/parties", response_model=Party, status_code=201, operation_id="addParty")
def add_party(payload: AddPartyInput, store: InMemoryStore = Depends(get_store)) -> Party:
...
def seat_party(
    partyId: UUID, payload: SeatPartyInput, store: InMemoryStore = Depends(get_store)
) -> SeatResult:
...
def mark_arrived(partyId: UUID, store: InMemoryStore = Depends(get_store)) -> Party:
    return store.mark_arrived(partyId)


@router.post("/parties/{partyId}/left", response_model=Party, operation_id="markLeft")
def mark_left(partyId: UUID, store: InMemoryStore = Depends(get_store)) -> Party:
...
def requeue_party(partyId: UUID, store: InMemoryStore = Depends(get_store)) -> Party:
