...
def list_tables(store: DatabaseStore = Depends(get_store)) -> list[Table]:
    return store.list_tables()


@router.get("/groups", response_model=list[TableGroup], operation_id="listGroups")
def list_groups(store: DatabaseStore = Depends(get_store)) -> list[TableGroup]:
...
def combine_tables(
    payload: CombineTablesInput, store: DatabaseStore = Depends(get_store)
) -> TableGroup:
...
def dissolve_group(groupId: UUID, store: DatabaseStore = Depends(get_store)) -> None:
