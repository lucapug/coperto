...
def get_shift(store: DatabaseStore = Depends(get_store)) -> Shift:
...
def open_shift(store: DatabaseStore = Depends(get_store)) -> Shift:
    return store.open_shift()


@router.post("/shift/close", response_model=Shift, operation_id="closeShift")
def close_shift(store: DatabaseStore = Depends(get_store)) -> Shift:
